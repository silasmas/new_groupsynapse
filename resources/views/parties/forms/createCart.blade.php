<div class="col-lg-8">
    <div class="checkout-wrap">
        <h5 class="title">Veuillez remplir le formulaire ci-dessous pour avoir une carte</h5>
        <small>
            ...
            <p class="text-danger">Le frais d'obtention de la carte est de {{ $serv->prix . '' . $serv->currency }}</p>
        </small>
...
@section('script')
    ...
    {{-- <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            // 🔄 Créer et afficher le loader dynamiquement
            const overlay = document.createElement('div');
            overlay.id = 'overlay-blurer';
            overlay.innerHTML = `<div class="loaderer"></div>`;
            ...
            // ✅ Réagir aux changements de la checkbox
            livraisonCheckbox.addEventListener('change', toggleAdresseDisplay);
            ...
            formInfo.addEventListener('submit', function(e) {
                e.preventDefault();
                // 🔄 Nettoyer anciennes erreurs visuelles
                document.querySelectorAll('.is-invalid, .input-error, .label-error, .error-bubble').forEach(
                    el => {
                        el.classList.remove('is-invalid', 'input-error', 'label-error');
                        if (el.classList.contains('error-bubble')) el.remove();
                ...
                const requiredFields = ['cnom', 'cemail', 'cphone', 'cpiece'];
                let valid = true;
                let messages = [];
                ...
                fetch("{{ route('init.service') }}", {
                    ...
                    .then(data => {
                        if (data.success) {
                            // ✅ Activer section paiement
                            paiementSection.style.opacity = 1;
                            paiementSection.style.pointerEvents = 'auto';
                            btnPaiement.removeAttribute('disabled');

                            // ✅ Injecter les valeurs reçues
                            document.getElementById('total').value = data.total;
                            document.getElementById('slug').value = data.slug;
                            document.getElementById('currency').value = data.currency;
                            document.getElementById('totalAff').textContent = data.total + data
                            ...
                            // Stocker l'ID de l'enregistrement
                            const userId = data.id;
                            const btnModifier = document.getElementById('btnModifier');

                            btnModifier.classList.remove('d-none'); // rendre visible
                            btnModifier.dataset.userId = userId; // stocker l'id pour l'utiliser après
                            paiement();
                            toastr.success(
                                "✅ Informations enregistrées. Vous pouvez passer à la caisse.");
                        } else {
            ...
            // 🔧 Fonction pour afficher une bulle d'erreur sous un champ
            function insertBubble(input, message) {
                const bubble = document.createElement('div');
                bubble.className = 'error-bubble';
                bubble.style.color = 'red';
            ...
            function paiement() {
                ...
                const checkboxTerms = document.getElementById(
                    "customCheck7"); // Case à cocher des conditions générales
